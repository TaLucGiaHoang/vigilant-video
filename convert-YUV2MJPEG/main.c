...
int main(void){
    ...
    */
    ...
    int numOfByte = width*height*3;
    memset(new_buffer, 0, numOfByte);
    unsigned char* buffer_char = new_buffer;
    unsigned int *buffer_int = buffer_start;    //4byte
    unsigned int _4_byte;

    for(i = 0; i < width*height; i+=2){
        //get 2 pixel in yuyv ( 4 bytes )
        _4_byte = buffer_int[i];
        BITMAP4 rgb1,rgb2;
        yuyv2rgb( &_4_byte, &rgb1, &rgb2);
        //add 2 new rgb pixel to new buffer ( 6 bytes )
        buffer_char[i] = rgb1.r;    //byte 0
        buffer_char[i+1] = rgb1.g;    //byte 1
        buffer_char[i+2] = rgb1.b;    //byte 2

        buffer_char[i+3] = rgb1.r;    //byte 3
        buffer_char[i+4] = rgb1.g;    //byte 4
        buffer_char[i+5] = rgb1.b;    //byte 5
        fprintf(textfile,"%d %d %d  %d %d %d  ",rgb1.r,rgb1.g,rgb1.b,rgb2.r,rgb2.g,rgb2.b);
        if(((i+1)%width)==0) fprintf(textfile,"\ni = %d\n",i);
	...
	write(imagefile, new_buffer, new_size);
	close(imagefile);
